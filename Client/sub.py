...
def on_message(client, userdata, message):
	aes_key_2, iv_2 = read_csv(userdata['csv_file_2'])
	print("Received message:", message.payload.decode())
	decrypted_message = decrypt_message(message.payload.decode(), aes_key_2, iv_2)
...
def decrypt_message(encrypted_message_base64, key, iv):
    key_bytes = bytes.fromhex(key)
    iv_bytes = bytes.fromhex(iv)
...
def main():
	...
	client.user_data_set({'csv_file_2': csv_file_2, 'topic': topic})
	# Set callbac
